...
from case002.models import Best
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('This program is "%s"' % 'case002-import-best.py'))
        ...
        def m3Best(src,sheetname):
            ...
            def doBest(k,title):
                ...
                for row in ws.rows:
                    ...
                    if rec == k:
                        A=get_val(row[0])
                        self.stdout.write(str(rec)+" "+A)
                        for key,val in dict1.items():
                            if get_val(row[val])=='xxx':
                                pass
                            else:
                                self.stdout.write(key+" "+str(val)+" "+title+ " " +get_val(row[val]))
                                Best(date1=key,title=title,name=get_val(row[val])).save()
                        break    
            doBest(4,'tt')            
            doBest(5,'role')            
            doBest(6,'speaker')            
               
        m3Best(src,'Registration')               
